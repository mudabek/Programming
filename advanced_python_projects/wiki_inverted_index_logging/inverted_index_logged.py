...
class InvertedIndex:
    ...
    def query(self, words: list):
        ...
        result.sort()
        logger.debug("query inverted index with %s", repr(words))
        return result
...
def callback_build(arguments):
    logger.debug(
        "call build with arguments: %s and %s", 
...
def callback_query(arguments):
    ...
    if arguments.queries is not None:
        logger.info("process queries: %s", repr(arguments.queries))
        for query in arguments.queries:
            callback_query_helper(query, inv_index)      
    else:
        logger.info("read queries from: %s", arguments.query_file)
