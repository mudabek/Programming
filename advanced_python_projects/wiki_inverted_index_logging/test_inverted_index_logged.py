from inverted_index_logged import (load_documents, InvertedIndex, 
...
def test_load_do__laods_properly():
...
def test_callback_query_do_process_queries_from_commandline_input(capsys,caplog):
    with caplog.at_level("DEBUG"):
        ...
        callback_query(query_args)
        captured = capsys.readouterr()
        assert any("query inverted index with" in message for message in caplog.messages), (
            "there is no 'query inverted index with' message in logs"
        )
        #possible also to use: logging._nameToLevel["WARNING"]
        assert all(record.levelno <= logging.WARNING for record in caplog.records), (
            "application is unstable, there are WARNING+ level messages in log"
        )
        assert "1" in captured.out
        assert "2,4" in captured.out, (
            "problem with callback query"
